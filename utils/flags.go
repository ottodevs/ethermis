...
var (
	...
	HostFlag = cli.StringFlag{
		Name:  "api_host",
		Usage: "The API server listening address",
		Value: "localhost",
	}

	PortFlag = cli.IntFlag{
		Name:  "api_port",
		Usage: "The API server listening port",
		Value: 8000,
	}
)
...
func RegisterEthService(ctx *cli.Context, stack *node.Node, extra []byte) {
	...
	if ethConf.LightMode {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			log.Fatalf("Failed to register the Ethereum light node service: %v", err)
		}
	} else {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			return ethereum.NewBackend(ctx, ethConf)
		}); err != nil {
			log.Fatalf("Failed to register the Ethereum full node service: %v", err)
		}
	}
}
...
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		log.Fatalf("Failed to register the Ethereum Stats service: %v", err)
	}
}
...
func MakeChainConfigFromDb(ctx *cli.Context, db ethdb.Database) *params.ChainConfig {
	...
	if genesis != nil {
		...
		default:
			log.Fatalf("Could not make chain configuration: %v", err)
		}
	}
...
// 	if err != nil {
// 		log.Fatalf("Could not start chainmanager: %v", err)
// 	}
